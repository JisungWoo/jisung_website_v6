...
export const koProjects = [
  {
    ...
    tags: ["헬스테크 SaaS", "워크플로우", "AI 세션 플래닝"],
    ...
    bullets: [
      "MelodyMap을 단순한 챗봇이나 기록 도구가 아니라 세션 준비, 진행, 사후 정리를 하나로 잇는 음악치료사용 워크플로우 플랫폼으로 정의했습니다.",
      "캘린더를 연동하면 방문 일정, 기관 정보, 준비 항목, 이동 기록이 자연스럽게 이어지고, 기관별로 쌓인 맥락을 다음 방문에도 활용할 수 있도록 설계하고 있습니다.",
      "프론트엔드는 TypeScript 기반 Next.js 15, 백엔드는 FastAPI로 구성했습니다. 서비스 레이어와 리포지토리 레이어를 분리하고 PostgreSQL, async SQLAlchemy, Alembic 중심의 운영 구조로 설계했습니다.",
      "세션 계획은 도입, 활동, 진행 스크립트, 변형, 마무리 등 블록 단위로 생성됩니다. PostgreSQL과 pgvector에 저장한 과거 세션 회고를 유사도 검색해 기관별 맥락이 다음 계획에 반영되도록 설계했습니다.",
      "벡터 유사도와 도메인 신호(대상군 특성, 치료사 레퍼토리, 과거 세션 결과)를 결합한 결정론적 추천 엔진을 추천 결과의 기준으로 삼고, LLM은 설명 문구만 생성하도록 역할을 분리해 임상 정보의 환각 위험을 낮췄습니다.",
